...
const documentTable = document.getElementById("tableCart")
...
function addcart(productImg, productName, productPrice) {
    ...
    // var cartItem=document.querySelectorAll("#resultCart tbody tr")
    var cartItem = documentTable.querySelectorAll("tbody tr")

    for (var i = 0; i < cartItem.length; i++) {
        var productT = documentTable.querySelectorAll(".title")
    ...
    var trcontent = '  <tr><td style="display: flex; align-items: center;">'
        + '<img style="width: 70;height: 70px;" src="' + productImg + '" alt="">'
        + '<span class="title">' + productName + '</span></td><td> <p>'
        + '<span class="price">' + productPrice + '</span><sup>đ</sup></p></td><td>'
        + '<input style="width: 30px; outline: none; " type="number" value="1" min="1" class="soluong"></td>'
        + '<td style="cursor: pointer;"><span class="cart-delete">Delete</span></td></tr>'
    // thêm

    addtr.innerHTML = trcontent
    // innerHTML là nội dung bên trong
    var cartTable = documentTable.querySelector("tbody")
...
function taoSuKienClickChoSPmoi() {

    var cartItem = documentTable.querySelectorAll("tbody tr")
    for (var i = 0; i < cartItem.length; i++) {
        var productT = documentTable.querySelectorAll(".cart-delete")
        //tạo lắng nghe sự kiện click nút xóa
        ...
        var productChange = documentTable.querySelectorAll(".soluong")
        //tạo lắng nghe sự kiện click nút thay đổi số lượng
        productChange[i].addEventListener("click", function (event) {
            //thay đổi số lượng thì chỉ tính tổng lại
            carttotal()
        })
